...
1. Image classification: just need to label a image as car
2. Classification with localization: label a image as car, put a bounding box around it.
3. Detection: multiple objects, detect them all and localize all.

* Classification with localization

1. softmax layer to decide pedestrian/car/motorcycle/background(nothing).
2. How to add localization? Add more layers and output 4 bounding box:

$$
b_x, b_y, b_h, b_w
$$

3. In this course, up left corner coordinates (0, 0), lower right (1, 1).
4. $b_x, b_y$ is the center of the bounding box.

* Defining the target label y

1. (1)pedestrian/(2)car/(3)motorcycle/background(nothing)
2. output $b_x, b_y, b_h, b_w$ class label (1-4)
...
5. The squared error is just for simplification, in practice, you can use
  log likely hood loss for $C_1, C_2, C_3$ and squared error for bounding box.
  For $P_c$ you can use logistic regression loss.

### 3-2 Landmark Detection

1. output, l stands for landmark.
...
2. Someone needs to go through and annotate these marks.
3. Can also work for pose detection.
4. Label has to be consistent across different images, meaning 1st point must be
  the left corner of the left eye, 2nd point must be right corner of the left eye.
...
1. Start with closely cropped images training set.
2. Then train a conv net, given cropped images, predict if it's a car or not.

* Sliding windows detection

1. Passing a squared region 1 by 1.
2. Slide the window until it goes through the entire image.
3. Resize the square region to bigger one.
4. Sliding the window over using some stride.
5. 3rd time using even larger windows.
6. Hopefully in some windows, you can find the car.
7. Huge disadvantage: computational cost.
8. If use big stride, then less image, but the coarse will hurt performance.
...
* Convolutional Implementation of Sliding Windows (Overfeat: Integrated recognition)
...
1. The resulted 2x2 is the same as doing 4 sliding window. So instead of doing forward
  propogation 4 times, we just need to do it once.
  
### 3-4 Bounding Box Predictions

1. The problem with section 3-3 is that it's not outputing the most accurate
  bounding box.

* output accurate bounding box

1. One problem is that none of the sliding window matches perfectly with the car

* YOLO algorithm: You only look once: Unified real-time object detection

1. First place a grid on the image
2. Labels for training, for each grid cell:
...
2. In the left and right image of 2nd row, it has car, YOLO algorithm will assign
  label for them. For the center image of 2nd row, although, it has partial cars,
  but since the center of the bounding box is not in them, YOLO algorithm will just
  assign don't care.
  
3. The target output will be 3x3x8, because we divide the image to 9 grids.
4. The advantage is the algorithm output precise bounding box.
5. In practice, you might use finer grid 19x19, reducing the chance of assigning
  multiple objects to the same grid.
6. The way you assign an object to grid cell as you look at the midpoint of an object and then you assign that object to whichever one grid cell contains the midpoint of the object.
7. Two things: this is more like image classification and localization algorithm.
8. second, this is a convolutional implementation. Just run convolution once.

* Specify the bounding boxes

1. For each grid, still the up left corner is (0, 0), lower right is (1,1)
2. There are more complex parameterization in the original paper
3. Warning: YOLO paper is hard to read even for Andrew
$$
y = \begin{bmatrix}
P_c \text{ (is there any object?) } \\
b_x \approx 0.4 \\
b_y \approx 0.3 \\
b_w = 0.9 \text{ could be greater than 1 } \\
b_h = 0.5 \\
C_1 \text{ (class 1?) } \\
C_2 \text{ (class 2?) } \\
C_3 \text{ (class 3?) } \\
...
1. How do you tell if your object detection algorithm is working well?
...
$$
\frac{\text{size of intersection (i.e. ground truth bounding box & the bounding box from prediction) }}
{\text{size of union}}
$$

2. "correct" if IoU >= 0.5, if want to be more stringent, then >= 0.6.
3. 0.5, 0.6 are just human chosen convention and no particular reason.
4. Andrew sometimes see people use more than 0.6, but rarely see use less than 0.5 .
5. More generally, IoU is a measure of the overlap between two bounding boxes.

### 3-6 Non-max Suppression

1. One problem with object detection: multiple detection of the same objects.
2. This algorithm makes sure detect each object only once.

* Non-max suppression example

1. Given 19x19 grid, the 2 car objects are bigger than the grid, so multiple grid
  would think it detects the car.
2. For each car object, algorithm might output multiple bounding box with different
  $P_c$, only hightlight the box with max $P_c$ and darken the others.

* Non-max suppression detail

1. Place a 19x19 grid on the picture, going to get 19x19x8 output.
2. But in this example, to simplify, you are only going to do car detection, so only
  get the following:
...
3. Discard all boxes with $P_c \le 0.6$
4. While there are any remaining boxes: 
    1. pick the box with the largest $P_c$, output that as a prediction.
    2. Discard any remaining boxes with IoU >= 0.5 with the box output in the previous
      step.
5. Previous example only has one object, if has 3 object, carry the algorithm 3 times,
  one on each of the output classes.

### 3-7 Anchor Boxes

1. One of the problems is that each of the grid can only detect one object.
2. How one grid cell detects multiple objects.

* Overlapping objects:

1. With this output, you can only output 1 class.
...
2. If stack 2 outputs together. First is for anchor box 1 and second is for anchor box 2.
...
1. Previously: each object in training image is assigned to grid cell that contains
  that object's midpoint.
    Output y: 3x3x8
2. With two anchor boxes: Each object in training image is assigned to grid cell that
  contains object's midpoint and anchor box for the grid cell with highest IoU.
    Output y: 3x3x16 or 3x3x2x8

* Anchor box example

1. If one grid has both people and car, then y would be:
...
3. Two corner cases, this algorithm doesn't handle well.
    1. If one grid has more than 2 objects
    2. If one grid has same shape of anchor boxes.
4. In practice, 2 objects in the same grid happens rarely, especially when we use
  19x19 grid.

### 3-8 YOLO Algorithm

* Training: You only look once: Unified real-time object detection
...
1. y is 3 x 3 x 2 (# of anchors) x 8 (5 + # of classes)
2. 
...
3. The upper left corner will be
...
5. In practice, it would be 19x19x16 grid, or even 19x19x5x8, which is 5 anchor boxes.
6. 

$$
100 \times 100 \times 3
\xrightarrow{\text{CONVNet}}
3 \times 3 \times 16
$$

* Makeing predictions

1. Hopefully the prediction of upper left corner will be:
...
2. Get rid of low probablity predictions.
3. For each class (pedestrian, car, motocycles) use non-max suppression to generate
  final predictions.

* YOLO algorithm summary

1. It encompasses many of the best ideas across the entire computer vision literature that relate to object detection.

### 3-9 Region Proposals

* Introduction

1. Andrew uses this algorithm a little bit less often, but it has been an influential body
  of work

* Region proposal: R-CNN (Rich feature hierachies for accurate object detection and
  semantic segmentation)

1. The downside of this algorithm that it crossfires a lot of regions that clearly has no
  objects.
2. Segmentation algorithm, what you do:
    1. find maybe 2000 blobs
    2. Place bounding boxes around the 2000 blobs
    3. run your classifiers on just those blobs
    4. Much smaller number than convolutional classifier

* Faster Algorithms

1. R-CNN is still quiet slow
2. R-CNN: Propose regions. Classify proposed regions one at a time. Output label + bounding box.
3. Fast R-CNN: Propose regions. Use convolution implementation of sliding windows to
  classify all the proposed regions.
4. Faster R-CNN: Towards real-time object detection with region proposal networks. Use
  convolutional network to propose regions. Andrew thinks it's still slower than YOLO
    algorithm.

* Conclusion

1. Andrew thinks YOLO is more promising than the 2-step region proposal algorithm.
...
1. Verification 1 to 1 problem
    1. Input image, name/ID
    2. Output whether the input image is that of the claimed person
2. Recognition (much harder 1 to K problem)
    1. Has a database of K persons
    2. Get an input image
    3. Output ID if the image is any of the K persons (or "not recognized")
3. We will first start with face verification system

### 4-2 One shot Learning

* Introduction

1. Challenge: need to recognize a person just one single image.
2. historically: deep learning doesn't work well if only have one training example.

* One-shot learning

1. Learning from one example to recognize the person again.
2. One approach: picture -> CNN -> softmax (5)
3. Doesn't work well, because training set is too small.
4. Also cannot handle the case when a new person joins the team

* Learning a similarity function

1. d(img1,img2) = degree of difference between images
2. d(img1,img2) $\le \tau$ "same"
3. d(img1,img2) $> \tau$ "different"

### 4-3 Siamese Network (DeepFace closing the gap to human level performance)

* Siamese network

1. Input image after convolution -> max pooling -> fully connected layer.
2. The fully connected layer is called "encoding of the picture"
3. Define $d(x^{(1)}, x^{(2)}) = || f(x^{(1)}) - f(x^{(2)})||_2^2$

* Goal of learning

1. Parameters of NN define an encoding $f(x^{(i)})$
2. Learn parameters so that:
    1. If same person, then distance is small
    2. If diff person, then distance is large

### 4-4 Triplet loss (FaceNet: A unified embedding for face recognition and clustering)

* Learning objective

1. Triplet loss: you'll always be looking at three images at a time.
2. Anchor image (A), Positive image (P), Negtive image (N)
3. $d(A,P) \le d(A,N)$
4. $||f(A) - f(P)||^2 - ||f(A) - f(N)||^2 \le 0$
5. A trivial solution would be having all f(img) = 0
6. To avoid this, $||f(A) - f(P)||^2 - ||f(A) - f(N)||^2 + \alpha \le 0$, $\alpha$ is called
    margin.

* Loss function

1. Given 3 images $A, P, N$
2. $L(A, P, N) = \max (||f(A) - f(P)||^2 - ||f(A) - f(N)||^2 + \alpha, 0)$
3. Cost function
...
4. Training set: 10k pictures of 1k persons. You do need multiple pictures for the same
  person.

* Choosing the triplets A, P, N

1. During training, if A,P,N are chosen randomly, $d(A,P)+\alpha \le d(A,N)$ is easily
satisfied.
2. Choose triplets that'are "hard" to train on.
3. $d(A,P) \approx d(A,N)$

* Training set using triplet loss

1. Commercial face recognition system trained at very large dataset. 10M, 100M images.
2. Some companies have trained on these sets and posted parameters online.

* Conclusion

1. Andrew feels it's a nice paper.

### 4-5 Face Verification and Binary Classification (DeepFace closing the gap to human level performance)

* Learning the similarity function

1. Let the 2 image pass through the siamese network and get 2 embedding vectors
2. Then feed these 2 vectors to a logistic regression unit and make it a binary
  classification problem.

$$
\hat{y} = \text{sigmoid} (\sum_{k=1}^{128} w_k |f(x^{(i)})_k - f(x^{(j)})_k| + b)
$$

3. $\chi^2$ formula, similarity

$$
\hat{y} = \text{sigmoid} (\sum_{k=1}^{128} w_k
\frac{(f(x^{(i)})_k - f(x^{(j)})_k)^2}{f(x^{(i)})_k + f(x^{(j)})_k} + b)
$$

4. For the image in the database, pre-compute the embeddings

### 4-6 What is neural style transfer

* Introduction

1. Content: Stanford university picture (C)
2. Style: Van Gogh's starry night (S)
3. Generated image (G)

4. Another example
5. Content: Golden gate bridge
6. Style: Picasso
7. Generated image
...
1. Pick a unit in layer 1. Find the nine image patches that maximize the unit's
activation.
2. Repeat for other units.
3. Read the paper, they have more sophisticated way of visualizing the ConvNet.
4. Later units are going to see larger images.

* Visualizing deep layers: Layer 2/3/4/5

1. Edge in layer 1
2. Texture in layer 2
3. More complex pattern in deeper layers.

### 4-8 Cost function

* Neural style transfer const function (A neural algorithm of artistic style)

1. $J(G) = \alpha J_{\text{content}}(C, G) + \beta J_{\text{style}}(S, G)$
2. The paper is not too hard to read, Andrew encourages to read it.

* Find the generated image G

1. Initiate G randomly: G: 100x100x3
2. Use gradient descent to minimize J(G): $G := G - \frac{\partial J(G)}{\partial G}$
...
1. $J(G) = \alpha J_{\text{content}}(C, G) + \beta J_{\text{style}}(S, G)$
2. Say you use hidden layer l to compute content cost
3. Use pre-trained ConvNet network (E.g. VGG network)
4. Let $a^{[l](C)}$ and $a^{[l](G)}$ be the activation of layer $l$ on the images.
5. If $a^{[l](C)}$ and $a^{[l](G)}$ are similar, both images have similar content.
6. $J_{\text{content}}(C, G) = \frac{1}{2} || a^{[l](C)} - a^{[l](G)} ||^2$

### 4-10 Style Cost Function

* Meaning of the "style" of an image

1. Say you are using layer l's activation to measure "style."
2. Define style as correlation between activations across channels.
3. How correlated are the activations across different channels $n_H \times n_W \times n_C$

* Intuition about style of an image

1. And so the correlation tells you which of these high level texture components tend to occur or not occur together in part of an image
2. Style matrix, $a_{i,j,k}^{[l]}$ = activation at $(i,j,k)$.
3. $G^{[l]}$ is $n_c^{l} \times n_c^{l}$, the matrix element $g_{kk'}^[l]$ is the 
  correlation between channel k and k', $k = 1, \dots, n_c$
...
4. In linear algebra, it's called "gram matrix"

$$
J_{\text{style}}(S, G) = ||G^{[L](S)} - G^{[L](G)}||^2_F \\
= \sum_k \sum_{k'} (G^{[L](S)} - G^{[L](G)})^2
$$

* Style cost function

$$
J_{\text{style}}^{[l]}(S, G) =  \frac{1}{(2n_H^{[l]}n_W^{[l]}n_C^{[l]})^2}
\sum_k \sum_{k'} (G^{[L](S)} - G^{[L](G)})^2 \\
...
1. The conv net can also be applied 1-D and 3-D data.

* Convolutions in 2D and 1D

$$
14 \times 14 \times 3
\xrightarrow[16]{5 \times 5 \times 3}
10 \times 10 \times 16
\xrightarrow[32]{5 \times 5 \times 16}
6 \times 6 \times 32
$$

2. EKG signal (heart beating)

$$
14 \times 1 \star 5 \times 1
\xrightarrow{6 \text{ filters}}
10 \times 16 \star 5 \times 16
\xrightarrow{} 6 \times 32
$$

* 3D data
* 3D convolution

1. 3D volumn * 3D filter

$$
14 \times 14 \times 14 \times 1
\xrightarrow[16 \text{ filters}]{5 \times 5 \times 5 \times 1}
10 \times 10 \times 10 \times 16
\xrightarrow[32 \text{ filters}]{5 \times 5 \times 5 \times 16}
6 \times 6 \times 6 \times 32
$$

### 4-12 Notes from Programming Exercises
